...
class Update
{
    public function update(Connection $connection, UpdateContext $updateContext): void
    {
        if (\version_compare($updateContext->getCurrentPluginVersion(), '1.0.0', '=')) {
            $this->updateTo110($connection);
        }
    }

    private function updateTo110(Connection $connection): void
    {
