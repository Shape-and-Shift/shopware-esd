import { EsdUploadEvents } from '../../../../service/esd.media.api.service';
import EsdMediaApiService from '../../../../service/esd.media.api.service';
 ...
 * @example code-only
 * @component-example
 * <sas-upload-listener @sas-uploads-added="..."></sas-upload-listener>
 */
Component.register('sas-upload-listener', {
    ...
    watch: {
        uploadTag(newVal, oldVal) {
            EsdMediaApiService.removeListener(oldVal, this.convertStoreEventToVueEvent);
            EsdMediaApiService.addListener(newVal, this.convertStoreEventToVueEvent);
        }
    },
    ...
    methods: {
        createdComponent() {
            EsdMediaApiService.addListener(this.uploadTag, this.convertStoreEventToVueEvent);
        },

        destroyedComponent() {
            EsdMediaApiService.removeListener(this.uploadTag, this.convertStoreEventToVueEvent);
        },
        ...
        convertStoreEventToVueEvent({ action, uploadTag, payload }) {
            ...
            if (action === EsdUploadEvents.UPLOAD_ADDED) {
                if (this.autoUpload === true) {
                    this.syncEntitiesAndRunUploads();
                    return;
                }

                this.$emit(EsdUploadEvents.UPLOAD_ADDED, payload);
                return;
            }

            if (action === EsdUploadEvents.UPLOAD_FINISHED) {
                this.updateSuccessNotification(uploadTag, payload);
                this.$emit(EsdUploadEvents.UPLOAD_FINISHED, payload);
                return;
            }

            if (action === EsdUploadEvents.UPLOAD_FAILED) {
                ...
                if (isDuplicationException(payload.error)) {
                    this.$emit(EsdUploadEvents.UPLOAD_FAILED, payload);
                    return;
                }
            ...
            if (action === EsdUploadEvents.UPLOAD_CANCELED) {
                this.$emit(EsdUploadEvents.UPLOAD_CANCELED, payload);
            }
        },
        ...
        syncEntitiesAndRunUploads() {
            EsdMediaApiService.runUploads(this.uploadTag);
        }
    }
});
